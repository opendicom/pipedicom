             ...
             do
                ...
                if [[ -z $STORERESP ]] || [[ $"STORERESP" == '<html><head><title>Error</title></head><body>Internal Server Error</body></html>' ]]; then
                   ...
                   if [[ $STORERESP == *"NativeDicomModel"* ]]; then
                      ...
                      echo $STORERESP | xsltproc --stringparam dstAET "$1" --stringparam org "$ORG" --stringparam dev "$SOURCE" --stringparam euid "$STUDY" --stringparam suid "$SERIES" /Users/Shared/opendicom/storedicom/respParsing.xsl - >> "$LOGPATH"
                      chmod 744 "$LOGPATH"
